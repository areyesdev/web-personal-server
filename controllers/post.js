...
function addPost(req, res) {
  ...
  post.save((err, postStored) => {
    if (err) {
      res.status(500).send({ code: 500, message: "Este post ya existe." });
    } else {
      if (!postStored) {
        res
          .status(400)
  ...
  Post.paginate({}, options, (err, postStored) => {
    if (err) {
      ...
      if (!postStored) {
        res
          .status(400)
          .send({ code: 400, message: "No se ha encontrado ningun post." });
      } else {
  ...
  Post.findByIdAndUpdate(id, postData, (err, postUpdate) => {
    if (err) {
      ...
      if (!postUpdate) {
        ...
        res
          .status(200)
          .send({ code: 200, message: "Post actualizado correctamentte." });
      }
    }
  });
}
